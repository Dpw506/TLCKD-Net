...
class HAM(nn.Module):  # The HAM is SFEM
...
class TLCKDNet(nn.Module):
    def __init__(self, cfg):
        ...
        self.ffs = nn.Sequential(
                                 ...
                                 nn.BatchNorm2d(96),
                                 nn.ReLU(inplace=True))   # MOGRM

        self.HAM = HAM(default_conv, 96, 3)
    ...
    def forward(self, x):
        ...
        x_sum1 = self.ffs(self.upsample2_(torch.cat([x_dec, x1],dim=1)))
