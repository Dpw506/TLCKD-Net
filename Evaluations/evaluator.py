...
class Eval_thread():
    ...
    def run(self):
        ...
        self.LOG(
            '{} ({}): {:.4f} mae || {:.4f} max-fm || {:.4f} S-measure || {:.4f} max-Em || {:.4f} AUC || {:.4f} Precision || {:.4f} Recall.\n'
            .format(self.dataset, self.method, mae, max_f, s, max_e, auc, p, r))
        return '[cost:{:.4f}s] {} ({}): {:.4f} mae || {:.4f} max-fm || {:.4f} S-measure || {:.4f} max-Em || {:.4f} AUC || {:.4f} Precision || {:.4f} Recall.'.format(
            time.time() - start_time, self.dataset, self.method, mae, max_f, s, max_e, auc, p, r)
